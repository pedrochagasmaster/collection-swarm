...
| `nim` | NVIDIA NIM inference API | `NVIDIA_NIM_API_KEY` (env var **or** dashboard `Settings → NVIDIA NIM` / `collection-swarm creds set nvidia_nim`) |
| `cursor_sdk` | Cursor SDK bridge (Node.js) | `CURSOR_API_KEY` (env var **or** dashboard `Settings → Cursor SDK` / `collection-swarm creds set cursor`), Node.js 22+ |
