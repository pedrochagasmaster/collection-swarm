...
!!! info "Domain roles vs. LLM roles"
    The domain uses `"collector"`, `"debtor"`, `"system"`, and `"judge"` roles (in the `Message` model). These are mapped to standard LLM roles (`"system"`, `"user"`, `"assistant"`) by each agent before being passed to the backend. See the [Debtor Agent docs](../agents/debtor.md#history-message-mapping) for an example of this mapping.

---

## Further Reading

- [LLM Router](router.md) — dispatch logic and backend registry
...
- [Agent Architecture](../agents/overview.md) — how agents use the backend system
