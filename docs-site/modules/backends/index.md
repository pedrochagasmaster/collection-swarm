...
| **NIM** | `NimBackend` | `backends/nim.py` | `NVIDIA_NIM_API_KEY` (env var **or** dashboard `creds set nvidia_nim`) | Production-grade LLM inference via NVIDIA NIM |
| **Cursor SDK** | `CursorSdkBackend` | `backends/cursor_sdk.py` | `CURSOR_API_KEY` (env var **or** dashboard `creds set cursor`) | Cursor coding agent API via Node.js bridge |

Both backends accept a [`CredentialResolver`](../credentials.md) via
their constructor. The router builds one per app session and reuses it
across every backend, so dashboard-stored keys override matching env
vars transparently.
